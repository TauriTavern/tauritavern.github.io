...
2. 如果你想使用或适配新的生成工作流，阅读 [Agent 总览](/agent/)。
3. 从 [架构总览](/architecture/overview) 建立整体模型。
4. 按需要进入 [前端集成](/architecture/frontend) 或 [后端分层](/architecture/backend)。
5. 当需要对外稳定说明时，再补充到 [API 文档](/api/)。
